...
- Per the skill's §5 (build state), offload each flow that needs a **new** counterpart to the `screen-builder` (→ Android) / `screen-builder-ios` (→ iOS) agent, giving it the source section node id, the ordered source screen node ids, and the **reserved rect** for placement. The agent builds each screen via `figma-sync:ios-to-android` / `figma-sync:android-to-ios` and **writes the identity stamp** on the new section. Reserved rects keep parallel agents from colliding.

## 10. Verify, stamp & record
- Screenshot each built or relocated section: confirm it sits in its canonical slot, unmanaged sections are untouched, and — for **builds** — each app-bar / nav title reads the flow name (not a content H1). On a mismatch, **restore from the layout backup** (and, for a failed build, remove the built node) and report the failure.
